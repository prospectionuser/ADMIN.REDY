  ...
  const uploadImage = async (file: File): Promise<string> => {
    ...
    if (uploadError) {
      if (uploadError.message.includes('row-level security policy')) {
        throw new Error("Erreur de permission Supabase : Vous devez configurer les politiques RLS pour le bucket 'product-images' dans votre console Supabase.");
      }
      throw uploadError;
    }
              ...
              <tr key={product.id} className="hover:bg-stone-50/50 transition-colors">
                ...
                <td className="px-6 py-4">
                  <div className="flex items-center gap-3">
                    <span className={`font-mono font-medium ${product.stock < 10 ? 'text-amber-600' : 'text-stone-900'}`}>
                      {product.stock}
                    </span>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 bg-stone-900/50 backdrop-blur-sm flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-2xl w-full max-w-lg shadow-xl overflow-hidden">
            <div className="px-6 md:px-8 py-6 border-b border-stone-100 flex items-center justify-between">
